...
credentials_data = json.loads(os.environ.get("CREDENTIALS_DATA"))
google_sheet_api = os.environ.get("GOOGLE_SHEET_API")
firebase_config = json.loads(os.environ.get("FIREBASE_CONFIG"))
service_account_key = json.loads(os.environ.get("SERVICE_ACCOUNT_KEY"))
...
# ----------------------------{pyrebase}---------------------------
pyre = pyrebase.initialize_app(firebase_config)
# -----------------------------------------------------------------
...
def signup(request: schemes.Signup):
    ...
    try:
        ...
        # send_mail.send_confirmation_email(email_to=email, name = name)
        assign_permission(senderEmail=email,name=name, plan='free_plan')
